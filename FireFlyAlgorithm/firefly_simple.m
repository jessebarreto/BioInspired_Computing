...
% ------------------------------------------------
alpha=0.86;      % Randomness 0--1 (highly random)
gamma=0.85;      % Absorption coefficient
delta=0.99;      % Randomness reduction (similar to 
...
for i=1:MaxGeneration,     %%%%% start iterations
    ...
    [xn,yn]=ffa_move(f,xn,yn,Lightn,xo,yo,Lighto,alpha,gamma,range);
    drawnow;
...
function [xn,yn]=ffa_move(f,xn,yn,Lightn,xo,yo,...
...
for i=1:ni,
    ...
    for j=1:nj,
        ...
        if Lightn(i)>Lighto(j), % Brighter and more attractive
            beta0=1;     beta=beta0*exp(-gamma*r); %beta=beta0*exp(-gamma*r.^2);
