...
% v 0.5
% Firefly Function Tester Parameters
% 
	...
	for d = 1:numel(N)
		...
		for f = 1:numel(functionNames)
            ...
            for experiment = 1:numberOfExperimentsPerParameters
                [spentTime, bestMinimumValue, bestMinimumPosition, bestMinimumValues] = fireflyFunction(functionName, searchSpace, dim, npar, maxIterations, threshold, dir, gamma, initialAlpha, finalAlpha, delta);
                ...
                title('Curva de convergencia Firefly S=' + string(npar) + ' N=' + string(dim) + ' ' + string(functionName));
            ...
            legend(plotFinal, 'Firefly Average');
            
            saveas(figure(figureNumber), char(string(string('ResultsFirefly/Firefly_S=') + string(npar) + string('_N=') + string(dim) + string('_') + string(functionName) + string('_') + string(timedate) + string('.fig'))));
...
save(char(string(string('ResultsFirefly/experimentValues_') + string(timedate) + string('.mat'))), 'experimentValues');
save(char(string(string('ResultsFirefly/averages_') + string(timedate) + string('.mat'))), 'averages');
save(char(string(string('ResultsFirefly/medians_') + string(timedate) + string('.mat'))), 'medians');
save(char(string(string('ResultsFirefly/mins_') + string(timedate) + string('.mat'))), 'minimums');
save(char(string(string('ResultsFirefly/stdDevs_') + string(timedate) + string('.mat'))), 'stdDevs');
save(char(string(string('ResultsFirefly/goalReachPercentage_') + string(timedate) + string('.mat'))), 'goalReachPercentage');
