	...
	for d = 1:numel(N)
		...
		for f = 1:numel(functionNames)
            ...
            for experiment = 1:numberOfExperimentsPerParameters
                [spentTime, bestMinimumValue, bestMinimumPosition, bestMinimumValues] = differentialEvolutionARFunction(functionName, searchSpace, dim, npar, maxIterations, threshold, dir, initialMutationFactor, finalMutationFactor, crossOverRate, divMin, divMax);
                ...
                title('Curva de convergencia DE AR S=' + string(npar) + ' N=' + string(dim) + ' ' + string(functionName));
            ...
            saveas(figure(figureNumber), char(string(string('ResultsDEAR/DE_S=') + string(npar) + string('_N=') + string(dim) + string('_') + string(functionName) + string('_') + string(timedate) + string('.fig'))));
...
save(char(string(string('ResultsDEAR/experimentValues_') + string(timedate) + string('.mat'))), 'experimentValues');
save(char(string(string('ResultsDEAR/averages_') + string(timedate) + string('.mat'))), 'averages');
save(char(string(string('ResultsDEAR/medians_') + string(timedate) + string('.mat'))), 'medians');
save(char(string(string('ResultsDEAR/mins_') + string(timedate) + string('.mat'))), 'minimums');
save(char(string(string('ResultsDEAR/stdDevs_') + string(timedate) + string('.mat'))), 'stdDevs');
save(char(string(string('ResultsDEAR/goalReachPercentage_') + string(timedate) + string('.mat'))), 'goalReachPercentage');
