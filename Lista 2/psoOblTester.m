...
% v 0.5
% PSO+OBL Function Tester Parameters
% 
	...
	for d = 1:numel(N)
		...
		for f = 1:numel(functionNames)
            ...
            for experiment = 1:numberOfExperimentsPerParameters
                [spentTime, bestMinimumValue, bestMinimumPosition, bestMinimumValues] = psoOblFunction(functionName, searchSpace, dim, npar, maxIterations, threshold, dir, vInitial, vMax, initialWeight, finalWeight, c1, c2, oblLimit);
            ...
            legend(plotFinal, 'PSOOBL Average');
            
            saveas(figure(figureNumber), char(string(string('ResultsPSOOBL/PSO_S=') + string(npar) + string('_N=') + string(dim) + string('_') + string(functionName) + string('_') + string(timedate) + string('.fig'))));
...
save(char(string(string('ResultsPSOOBL/experimentValues_') + string(timedate) + string('.mat'))), 'experimentValues');
save(char(string(string('ResultsPSOOBL/averages_') + string(timedate) + string('.mat'))), 'averages');
save(char(string(string('ResultsPSOOBL/medians_') + string(timedate) + string('.mat'))), 'medians');
save(char(string(string('ResultsPSOOBL/mins_') + string(timedate) + string('.mat'))), 'minimums');
save(char(string(string('ResultsPSOOBL/stdDevs_') + string(timedate) + string('.mat'))), 'stdDevs');
save(char(string(string('ResultsPSOOBL/goalReachPercentage_') + string(timedate) + string('.mat'))), 'goalReachPercentage');
