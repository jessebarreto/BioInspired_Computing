	...
	for d = 1:numel(N)
		...
		for f = 1:numel(functionNames)
            ...
            for experiment = 1:numberOfExperimentsPerParameters
                [spentTime, bestMinimumValue, bestMinimumPosition, bestMinimumValues] = differentialEvolutionFunction(functionName, searchSpace, dim, npar, maxIterations, threshold, dir, initialMutationFactor, finalMutationFactor, crossOverRate);
                ...
                title('Curva de convergencia DE S=' + string(npar) + ' N=' + string(dim) + ' ' + string(functionName));
            ...
            saveas(figure(figureNumber), char(string(string('ResultsDE/DE_S=') + string(npar) + string('_N=') + string(dim) + string('_') + string(functionName) + string('_') + string(timedate) + string('.fig'))));
