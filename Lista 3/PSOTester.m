...
% v 0.5
% PSO PID Tunning tester.
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [] = PSOTester(algorithmParameters, maxIterations, numberOfExperimentsPerParameters, plantName);
	...
	% PSO Parameters [1 1.2 0.9 50 5 0.2 1.2]
	% Direction
		...
		for d = 1:numel(N)
	        dimensions = N(d);
				...
				for experiment = 1:numberOfExperimentsPerParameters
					[spentTime, bestMinimumValue, bestMinimumPosition, bestMinimumValues] = psoPIDTunning(plantName, systemTF, systemNoiseTF, ...
...
%                     plantEvaluator(plantName, bestMinimumPosition);
                    ...
                    title('Curva de convergencia PSO S=' + string(npar) + ' N=' + string(dim) + ' ' + string(plantName));
                ...
                saveas(figure(figureNumber), char(string(string('ResultsPSO/PSO_S=') + string(npar) + string('_N=') + string(dim) + string('_') + string(plantName) + string('_') + string(timedate) + string('.fig'))));
    ...
    save(char(string(string('ResultsPSO/experimentValues_') + string(timedate) + string('.mat'))), 'experimentValues');
	...
	save(char(string(string('ResultsPSO/minsPID_') + string(timedate) + string('.mat'))), 'minimumPID');
