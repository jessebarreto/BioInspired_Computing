...
% v 0.5
% ODE PID Tunning tester.
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [] = ODETester(algorithmParameters, maxIterations, numberOfExperimentsPerParameters, plantName);
	...
	% DE Parameters [1 1.0 1.35 0.95 40]
	% Direction
    ...
    oblLimit = algorithmParameters(5);
		...
		for d = 1:numel(N)
	        dimensions = N(d);
				...
				for experiment = 1:numberOfExperimentsPerParameters
					[spentTime, bestMinimumValue, bestMinimumPosition, bestMinimumValues] = odePIDTunning(plantName, ...
...
%                     plantEvaluator(plantName, bestMinimumPosition);
                    ...
                    title('Curva de convergencia ODE S=' + string(npar) + ' N=' + string(dim) + ' ' + string(plantName));
    ...
    save(char(string(string('ResultsODE/experimentValues_') + string(timedate) + string('.mat'))), 'experimentValues');
	save(char(string(string('ResultsODE/averages_') + string(timedate) + string('.mat'))), 'averages');
	save(char(string(string('ResultsODE/medians_') + string(timedate) + string('.mat'))), 'medians');
	save(char(string(string('ResultsODE/mins_') + string(timedate) + string('.mat'))), 'minimums');
	save(char(string(string('ResultsODE/minsPID_') + string(timedate) + string('.mat'))), 'minimumPID');
	save(char(string(string('ResultsODE/stdDevs_') + string(timedate) + string('.mat'))), 'stdDevs');
	save(char(string(string('ResultsODE/goalReachPercentage_') + string(timedate) + string('.mat'))), 'goalReachPercentage');
